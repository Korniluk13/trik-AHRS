...
double MatrixIntegrator::getRoll(const QQuaternion &q) const
{
	return asin(-2 * q.x() * q.z()+2 * q.scalar() * q.y()) * mRadToDeg;
}

double MatrixIntegrator::getPitch(const QQuaternion &q) const
{
...
double MatrixIntegrator::getYaw(const QQuaternion &q) const
{
